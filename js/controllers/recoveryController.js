document.addEventListener('DOMContentLoaded', () => {
  ...
  form?.addEventListener('submit', async (event) => {
    ...
    if (!email) {
      Swal.fire('Error', 'Escribe tu correo institucional.', 'error');
      return;
    }

    if (currentStep === 1) {
      // Step 1: Request OTP
      const endpoint = 'http://localhost:8080/api/studentsAuth/requestPasswordOtp';
      ...
      const endpoint = 'http://localhost:8080/api/studentsAuth/resetPasswordWithOtp';
